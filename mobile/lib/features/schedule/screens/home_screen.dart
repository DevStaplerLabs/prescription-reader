...
class _TestReminderButtonState extends State<_TestReminderButton> {
  ...
  Future<void> _triggerTestReminder(BuildContext context, WidgetRef ref) async {
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Failed to send test reminder: $e',
              style: GoogleFonts.plusJakartaSans(color: Colors.white, fontWeight: FontWeight.bold),
            ),
            backgroundColor: AppTheme.dangerColor,
            behavior: SnackBarBehavior.floating,
      ...
      builder: (context, ref, child) {
        return MouseRegion(
          ...
          child: GestureDetector(
            ...
            onTapUp: (_) => setState(() => _isPressed = false),
            onTapCancel: () => setState(() => _isPressed = false),
            onTap: _isLoading ? null : () => _triggerTestReminder(context, ref),
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 200),
              curve: Curves.easeInOut,
              transform: Matrix4.diagonal3Values(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(
                    _isLoading ? 'Sending Test Reminder...' : 'Set Reminder Now (Test WhatsApp)',
                    style: GoogleFonts.plusJakartaSans(
                      color: AppTheme.primaryColor,
                      fontWeight: FontWeight.bold,
                      fontSize: 12,
